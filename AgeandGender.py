...
#     recognizer.prepare(ctx_id=0, nms=0.4)  # Use GPU (ctx_id=0) or CPU (ctx_id=-1)
#
#     return recognizer
    ...
    while True:
        ...
        for face in faces:
            ...
            if highest_similarity > threshold:
                label = f"{best_match} (Gender: {gender}, Age: {age}, Sim: {highest_similarity:.2f})"
            else:
                label = f"Unknown (Gender: {gender}, Age: {age})"

            # Draw the bounding box around the face
            cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 255, 0), 2)
            # Draw the name label above the face
            cv2.putText(frame, label, (bbox[0], bbox[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (36, 255, 12), 2)
